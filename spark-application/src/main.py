...
""" def load_config(spark_context: SparkContext):
    spark_context._jsc.hadoopConfiguration().set(
        "fs.s3a.access.key", os.getenv(
            "AWS_ACCESS_KEY_ID", "<Your-MinIO-AccessKey>")
    )
    spark_context._jsc.hadoopConfiguration().set(
        "fs.s3a.secret.key",
    ...
    spark_context._jsc.hadoopConfiguration().set(
        "fs.s3a.endpoint", os.getenv("ENDPOINT", "<Your-MinIO-Endpoint>")
    )
    spark_context._jsc.hadoopConfiguration().set(
...
df = (
    ...
    .schema(schema)
    .csv(os.getenv(
        "INPUT_PATH", "s3a://landing-zone/2018_Yellow_Taxi_Trip_Data.csv"))
)
# Filter dataframe based on passenger_count greater than 5
large_passengers_df = df.filter(df.passenger_count > 5)
total_rows_count = df.count()
filtered_rows_count = large_passengers_df.count()


# File Output Committer is used to write the output to the destination path
large_passengers_df.write.format("csv").option("header", "true").save(
    os.getenv(
        "OUTPUT_PATH", "s3a://bronze/spark/nyc/taxis_small")
)
